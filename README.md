...
Replicable for European cities : Firstly, the previous version was hard-coded for Ostend at several places (city coordinates, GADM municipality name, country code, admin level). All city-specific parameters have now been centralised in a single block at the top of the preprocessing file, and the rest of the script reads from these variables. Secondly, improving the cartographic elements adding to the Leaflet map a north arrow. Finalaly, The codebase is now fully commented inline, with every helper function preceded by a short explanation of its purpose, inputs and side-effects.
